...
export default function AddBeer(){
    ...
    const [beerActiveCheck, setBeerActiveCheck] = useState(false);

    const handleCheckBoxChange = (e) => {
        setBeerActiveCheck(e.target.value);
        if(beerActiveCheck){
            values.active = beerActiveCheck;
        }else{
            values.active = "0"
        }
    }
...
/*<FormControl component="fieldset" className={classes.formControl}>
                    <FormGroup>
                        <FormControlLabel
                            control={<Checkbox value={1} onChange={handleCheckBoxChange} color="#FFA200" />}
                            label="Beer is inactive"
                        />
                    </FormGroup>
                    <FormHelperText>Check this box if beer is no longer being sold</FormHelperText>
                </FormControl>*/